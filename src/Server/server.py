...
# Randomly generate coordinates for each ergot sample in its district
...
# Load models - there are plenty more but they are slow to load (different functions can be called from Server/logic/modelLoader)
models = getForestModels()
loadedModelInfo = []

# Numpy numbers do not like to be jsonified - formats data for the frontend
for model in models:
    modelStatistics = {
...
# Flask folders have their own way of searching for files (starts at where the flask script is located)
