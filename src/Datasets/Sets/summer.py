...
# summer.py
#
# After loading all datasets then aggregating them, the following class can be used to create exploratory datasets from the summer data
# Summer data appears in the following date ranges:
#   - Daily: May 1st - August 31st (inclsuive)
#   - Weekly: Week 17 - Week 32 (inclusive)
#   - Monthly: Month 5 - Month 8 (inclusive)
#
...
#
# Remarks:
#   - the columns summer sets expect from input data to run properly are:
#       - year
#       - district
    ...
    def selectData(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(df) > 0:
            ...
            if lastAttr.find("-") != -1:  # This is a daily aggregate
                df = df[
                    df.columns.drop(list(df.filter(regex="^0[1-4]-|^09-|^1[0-2]-")))
                ]
            elif lastAttr.find("12:") != -1:  # This is a monthly aggregate
                df = df[df.columns.drop(list(df.filter(regex="^[1-4]:|^9:|^1[0-2]:")))]
            elif lastAttr.find("52:") != -1:  # This is a weekly aggregate
                df = df[
                    df.columns.drop(
                        list(
                            df.filter(
                                regex="^[1-9]:|^1[0-6]:|^3[3-9]:|^4[0-9]:|^5[0-2]:"
                            )
                        )
                    )
                ]
